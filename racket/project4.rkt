...
; Define Reverse funtion : one argument, empty list
...
(define (palindrome list)
	...
	(let helper ((orig list) (aux list) (half '()))
			; Test conditions: Checks if aux is null 
			(cond ((null? aux)       
				; Checks if half and orig are equal
				(equal? half orig))
					; Checks if the next characters in aux are null
					((null? (cdr aux)) 
						; Checks if half and the rest of orig are equal
						(equal? half (cdr orig)))
							...
							(else (helper (cdr orig) (cddr aux)
								; Constructs half, placing an element in front of the next in a list
								(cons (car orig) half)
			)))))
